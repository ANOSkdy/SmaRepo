...
function watchForBetter(opts: {
  limitMs: number;
  earlyStop: (p: Fix) => boolean;
}): Promise<Fix | null> {
...
async function getBestPositionAdaptive(
  ...
  try {
    ...
    const budget =
      a1 > VERY_BAD_ACC && age1 > VERY_OLD_MS ? HARD_MAX_WAIT : SOFT_MAX_WAIT;
...
export default function StampCard({
  ...
  const [stampType, setStampType] = useState<'IN' | 'OUT' | 'COMPLETED'>(
    initialStampType
  );
  ...
  const machineId =
    searchParams.get('machineId') ?? searchParams.get('machineid');
  ...
  const handleStamp = useCallback(
    async (
      ...
      options?: {
        ...
        nextStampState?: 'IN' | 'OUT' | 'COMPLETED';
      }
    ): Promise<boolean> => {
      const effectiveMachineId = (
        options?.machineIdOverride ?? machineId
      )?.trim();
      ...
      try {
        ...
        if (position.source === 'cache') {
          warnings.push(
            '最新の位置情報を取得できなかったため、最後に保存した位置情報を使用しました。'
          );
        }
        ...
        try {
          ...
          if (
            ...
            combinedWarnings.push(
              '録拠点から離れている可能性があります（>1km）'
            );
          }
          setWarning(combinedWarnings.join(' / '));
          if (!response.ok) {
            throw new Error(
              data.message || `サーバーエラー: ${response.statusText}`
            );
          }
          if (type === 'IN') {
            setStampType('OUT');
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (success) {
        ...
        setLocationError(null);
        router.replace(
          `/nfc?machineId=${encodeURIComponent(currentMachineId)}`
        );
      }
    })();
  }, [
  ...
  if (error) {
    return (
      <CardState title="エラーが発生しました" message={error} role="alert" />
    );
  }
  if (!machineId) {
    return (
      <CardState
        title="無効なアクセス"
        message="NFCタグから機械IDを読み取れませんでした。"
        role="alert"
      />
    );
  }
  ...
  return (
    ...
    >
      {warning ? (
        <div
          role="status"
          className="bg-brand-primary/10 w-full rounded-lg border border-brand-border px-4 py-2 text-sm text-brand-text"
        >
          {warning}
        </div>
      ) : null}
