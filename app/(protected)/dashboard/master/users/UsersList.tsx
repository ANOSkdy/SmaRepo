    ...
    <div className="space-y-4">
      <form onSubmit={onSubmit} className="space-y-3 rounded-lg border border-brand-border bg-brand-surface p-4 text-sm text-brand-text">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          ...
          {!editingId ? <label className="space-y-1"><span>パスワード</span><input required minLength={8} type="password" className="w-full rounded border border-brand-border px-2 py-1" value={form.password} onChange={(e) => setForm((prev) => ({ ...prev, password: e.target.value }))} /></label> : null}
